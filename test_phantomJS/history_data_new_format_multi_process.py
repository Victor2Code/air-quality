...
def one_province(province):
    #province是一个string，用空格隔开分别是url，省的名字和AQI
    ...
    with open(province.split(' ')[1]+'.csv','a') as f:
        ...
        # except Exception as e:
            # print(str(e))
            # with open('download_result.txt','a') as f:
                # f.write(province.split(' ')[1]+'省 下载【失败】'+'\n')
            # driver.quit()
            # return "fail"
    with open('download_result.txt','a') as f:
        f.write(province.split(' ')[1]+'省 下载【成功】'+'\n')
    driver.quit()
    return "success"
...
def main(url,charset):
    ...
    #多进程是按照每个省一个浏览器的方式，将one_province函数放入进程池
    total_provinces=[]
    ...
    if not os.path.isfile('download_result.txt'):
        os.mknod('download_result.txt')
    else:
        os.remove('download_result.txt')
        os.mknod('download_result.txt')
    #默认权限是700
    print('>> 下载结果保存在 download_result.txt 文件中')
    print('=== === === === === ===')
    print('>> 创建进程池，同时下载 3 个省的数据')
    pool=multiprocessing.Pool(processes=3)
    result=pool.map(one_province,total_provinces)
    pool.close()
    pool.join()
    return result 
